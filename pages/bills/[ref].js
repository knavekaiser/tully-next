...
import { CostingPrint } from "pages/costings/[lot]";
...
class Bill_Class extends Component {
  render() {
    ...
    const costing = this.props.costingToPrint
      ? (bill.costings || []).find(
          (item) => item.lot === this.props.costingToPrint
        )
      : null || null;
    const wages = {};
  ...
  const fetchData = (lotNo) => {
    fetch(`/api/bills?ref=${router.query.ref}&includeCostings=true`)
      ...
      .then((data) => {
        if (data.code === "ok") {
          setData(data.bill);
          setCostingToPrint(data.bill.costings?.[0]?.lot || null);
  ...
  useEffect(() => {
    if (SS.get("singleBillData")) {
      ...
      if (localData.ref === +router.query.ref) {
        setData(localData);
        setCostingToPrint(localData.costings?.[0]?.lot || null);
      } else {
        fetchData();
      }
    } else {
      ...
      >
        ...
        <tr>
          ...
          {data?.costings?.length > 0 && (
            <td>
              <select
                defaultValue={costingToPrint}
                onChange={(e) => {
                  setCostingToPrint(+e.target.value);
                }}
              >
