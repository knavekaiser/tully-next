...
import { AddSeason, AddGroup } from "./comp/form";
import {
  IoLockClosedOutline,
...
export default function Productions() {
  ...
  const { user, setNameTag, seasons, setSeasons, groups, setGroups } =
                  ...
                  <button
                    onClick={() => {
                      if (confirm("You sure want to delete this season?")) {
                        fetch(`/api/season/${item._id}`, { method: "DELETE" })
                          .then((res) => res.json())
                          .then(({ code }) => {
                            if (code === "ok") {
                              setSeasons((prev) =>
                  ...
                  <button
                    onClick={() => {
                      if (confirm("You sure want to delete this group?")) {
                        fetch(`/api/groups/${item._id}`, { method: "DELETE" })
                          .then((res) => res.json())
                          .then(({ code }) => {
                            if (code === "ok") {
                              setGroups((prev) =>
